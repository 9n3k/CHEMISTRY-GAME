...
from game import play_intro_video
...
def main_menu():
    ...
    # Start the music AFTER the intro screen
    try:
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                sys.exit()
            if event.type == pygame.MOUSEBUTTONDOWN:
                # In menu.py, modify the PLAY_BUTTON section:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        # In your existing code, modify the PLAY_BUTTON section to look like this:
                            # In your main_menu function, modify the PLAY_BUTTON section to:
                            if PLAY_BUTTON.checkForInput(MENU_MOUSE_POS):
                                ...
                                # Show loading screen
                                loading_screen = pygame.Surface((1280, 720))
                                loading_screen.fill((0, 0, 0))
                                loading_text = get_font(30).render("Loading...", True, (255, 255, 255))
                                loading_screen.blit(loading_text, (640 - loading_text.get_width()//2, 360))
                                ...
                                # After video finishes, start game with fresh audio
                                if video_played:
                                    pygame.mixer.quit()
